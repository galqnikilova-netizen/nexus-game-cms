function translateIfNeeded(text, module) {
    return (typeof text !== 'undefined' && text !== null && text.length > 0 && text[0] === '_') 
        ? get_translate_module_phrase(module, text) 
        : text;
}
...
function fillEditPriceOptionsTable(data) {
    ...
    $('#edit-price-options').find('.shop_label_price').fadeOut();
    $('#edit-price-options').find('.shop_price_options').fadeOut();
    $('#edit-price-options').find('.shop_tx_0').fadeIn();
    $('#edit-price-options').find('.shop_tx_' + type).fadeIn();
}
...
function createProperty(id, serverId, active, title, sort, type = 'table') {
    ...
    let displayTitle = translateIfNeeded(title, 'module_page_store');
    let result =
        '<form id="edit-' + type + '-property_' + id + '" data-sort="' + sort + '" data-original-title="' + title + '" class="shop_product_property ' + className + ' shop_table_property">' +
        ...
        '<div class="shop_product_property_title">' + displayTitle + '</div>' +
        '</form>';
    return result;
}

function changePropertyField(id, type = 'table') {
    let formElement = $('#edit-' + type + '-property_' + id);
    let title = formElement.attr('data-original-title');
    let translatedTitle = translateIfNeeded(title, 'module_page_store');
    let checked = formElement.hasClass('shop_product_property_disabled') ? '' : 'checked';
    let sort = formElement.attr('data-sort');
    let elements =
        '<div class="shop_product_' + type + '_property_icon_delete" onclick="sendAjax(\'delete-' + type + '-property\', ' + id + ')">' +
        '<svg viewBox="0 0 448 512"><path d="M135.2 17.7L128 32H32C14.3 32 0 46.3 0 64S14.3 96 32 96H416c17.7 0 32-14.3 32-32s-14.3-32-32-32H320l-7.2-14.3C307.4 6.8 296.3 0 284.2 0H163.8c-12.1 0-23.2 6.8-28.6 17.7zM416 128H32L53.2 467c1.6 25.3 22.6 45 47.9 45H346.9c25.3 0 46.3-19.7 47.9-45L416 128z" /></svg></div>' +
...
function setPrice(serverId, id) {
    ...
    $('#shop_product_description_price_' + serverId).find('.shop_product_color_value_' + serverId).text(price + value);
    $('#shop_product_price_' + serverId).find('.shop_product_price_count').text(price);
    if (price != price_old) {
        $('#shop_product_price_' + serverId).find('.shop_product_price_old').text(price_old + ' ' + value);
    }
    $('#shop_product_price_' + serverId).find('.shop_product_color_value_' + serverId).text(value);
}
...
function showAddPriceTable(productId) {
    ...
    $('#add-price-options').find('.shop_label_price').hide();
    $('#add-price-options').find('.shop_price_options').hide();
    $('#add-price-options').find('.shop_tx_0').show();
    $('#add-price-options').find('.shop_tx_' + type).show();
...
function showCatalog(id) {
    ...
    $(".shop_server_active").removeClass("active");
    let serverText = $('.shop_server_' + id).text();
    document.getElementById('name_server').textContent = translateIfNeeded(serverText, 'module_page_store');
    $(".shop_server_" + id).addClass("active");
    $(".wrapper_server_accept").hide();
    $(".shop_black_screen").hide();
    $(".filter-category").removeClass("active");
    $(".shop_cat_0").addClass("active");
}
...
$(document).ready(function() {
    $('.filter__by-price button').click(function() {
        $('.filter__by-price button').removeClass('active');
        $(this).addClass('active');
        const sort = $(this).data('sort');
        $.ajax({
            type: 'POST',
            url: location.href,
            data: { store_sort: sort },
            dataType: "json",
            success: function(jsonData) {
