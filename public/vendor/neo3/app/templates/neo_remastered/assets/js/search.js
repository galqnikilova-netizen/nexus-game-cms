...
$('input[name="_steam_id"]').on("input", (e) => {
    ...
    $("#search_header").html('<div class="load"><svg><use href="/vendor/neo3/resources/img/sprite.svg#spinner"></use></svg> '+get_translate_phrase('_lookingPlayer')+' </div>');
})
...
function searchFromModule( input = "" )
{
    ...
    (async () => {
        ...
        if( json.result?.error )
        {
            modal.removeClass("users_found");
            $("#search_header").html(`
                <svg><use href="/vendor/neo3/resources/img/sprite.svg#x"></use></svg>`+json.result?.error);
        ...
        if( json.result?.players.length && json.result?.players.length > 0 )
        {
            ...
            $("#search_header").html(`
                <svg><use href="/vendor/neo3/resources/img/sprite.svg#magnifying-glass"></use></svg>
            ...
            for( let player of json.result?.players )
            {
                $(".modal_blocks_content").append(`<a href="${domain}profiles/`+player.steam64+`?search=1">
                    ...
                    <svg><use href="/vendor/neo3/resources/img/sprite.svg#arrow-top-tight-circle"></use></svg>
                </a>`)
            }
        }
        else
        {
            modal.removeClass("users_found");
            $("#search_header").html(`
            <svg><use href="/vendor/neo3/resources/img/sprite.svg#x"></use></svg>
